<template>
  <ProfileLayout>
    ...
    <v-expand-transition>
      <PersonalInformation v-if="(menuSelected === 'personal')" />
      <InsuranceInformation v-if="(menuSelected === 'insurance')" />
      <AddressInformation v-if="(menuSelected === 'address')" />
    </v-expand-transition>
  </ProfileLayout>
</template>
...
import type { LocationQueryValue } from 'vue-router';
  ...
  const menuList = [
    ...
    { title: 'Insurance Information', icon: 'mdi-card-account-details-outline', value: 'insurance' },
    { title: 'Address', icon: 'mdi-map-marker-outline', value: 'address' }
  ];

  // Data
  const menuSelected = ref<string | LocationQueryValue | LocationQueryValue[]>((new RegExp(`^(${menuList.map((nav) => (nav.value)).join('|')})$`, 'ig').test(route.query?.view as string)) ? route.query.view : 'personal');
