<template>
  <ProfileLayout>
    ...
    <v-expand-transition>
      <MyPastAppointments v-if="(menuSelected === 'my-appointments')" />
      <FamilyMembersPastAppointments v-if="(menuSelected === 'family-members')" />
    </v-expand-transition>
  </ProfileLayout>
</template>
...
<script lang="ts" setup>
  ...
  // Static Data
  const menuList = [
    { title: 'My Appointments', icon: 'mdi-account-outline', value: 'my-appointments' },
    { title: 'Family Members', icon: 'mdi-account-multiple-outline', value: 'family-members' }
  ];

  // Data
  const menuSelected = ref<string | LocationQueryValue | LocationQueryValue[]>((new RegExp(`^(${menuList.map((nav) => (nav.value)).join('|')})$`, 'ig').test(route.query?.view as string)) ? route.query.view : 'my-appointments');
